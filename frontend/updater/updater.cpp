...
static void ClearShaderCache()
{
	...
	if (SHGetFolderPathW(NULL, CSIDL_COMMON_APPDATA, NULL, SHGFP_TYPE_CURRENT, shader_path) == S_OK) {
		if (SUCCEEDED(StringCbCatW(shader_path, sizeof(shader_path), L"\\obs-12tracks-multilingual\\shader-cache"))) {
...
static bool Update(wchar_t *cmdLine)
{
	...
	StringCbCat(lpAppDataPath, sizeof(lpAppDataPath), L"\\obs-12tracks-multilingual");
	...
	if (!GetTempFileNameW(tempDirName, L"obs-12tracks-multilingual", 0, tempPath)) {
