...
try {
	...
	if (FetchAndVerifyFile("branches", "obs-12tracks-multilingual\\updates\\branches.json", WIN_BRANCHES_URL, &text))
	...
	text.clear();
	if (!FetchAndVerifyFile("manifest", "obs-12tracks-multilingual\\updates\\manifest.json", manifestUrl.c_str(), &text,
				extraHeaders))
		return;
	...
	if (!FetchAndVerifyFile("updater", "obs-12tracks-multilingual\\updates\\updater.exe", WIN_UPDATER_URL, nullptr))
		return;
	...
	BPtr<char> updateFilePath = GetAppConfigPathPtr("obs-12tracks-multilingual\\updates\\updater.exe");
